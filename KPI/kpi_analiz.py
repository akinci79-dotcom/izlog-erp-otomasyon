...
def _gelir_kpi(cursor, bas, bit):
    ...
    cursor.execute(
        ...
        SELECT
            ...
            NVL(SUM(CASE
                WHEN OPDET.INVOICE_M_ID IS NULL AND OPDET.PURCHASE_SALES_TYPE IN (2, 4)
                 AND NVL(HK.EXPENSE_CODE, ' ') NOT IN ('ALDIĞI MAZOT TL', 'ELDEN')
                THEN OPDET.AMT ELSE 0 END), 0) AS FATURASIZ_TUTAR,
            NVL(SUM(CASE
                WHEN OPDET.INVOICE_M_ID IS NOT NULL AND OPDET.PURCHASE_SALES_TYPE IN (2, 4)
                 AND NVL(HK.EXPENSE_CODE, ' ') NOT IN ('ALDIĞI MAZOT TL', 'ELDEN')
